...
describe Musicality::Note do
  ...
  it "should be constructible (no raised exceptions) without the optional parameters" do
    lambda { Musicality::Note.new :pitch => @pitch, :duration => 1 }.should_not raise_error ArgumentError
    lambda { Musicality::Note.new :pitch => @pitch, :duration => 1 }.should_not raise_error RangeError
  end
  
  it "should assign pitch and duration parameters when given during construction" do
    note = Musicality::Note.new :pitch => @pitch, :duration => 2
  ...
  it "should assign :sustain, :attack, and :seperation parameters if given during construction" do
    note = Musicality::Note.new :pitch => @pitch, :duration => 2, :sustain => 0.1, :attack => 0.2, :seperation => 0.3
  ...
  it "should assign :relationship parameter if given during construction" do
    note = Musicality::Note.new :pitch => @pitch, :duration => 2, :relationship => Musicality::Note::RELATIONSHIP_TIE
    note.relationship.should eq(Musicality::Note::RELATIONSHIP_TIE)

    note = Musicality::Note.new :pitch => @pitch, :duration => 2, :relationship => Musicality::Note::RELATIONSHIP_SLUR
  ...
  it "should be hash-makeable" do
    ...
    hash = { :pitch => {:octave => 2}, :duration => 2, :relationship => Musicality::Note::RELATIONSHIP_TIE }
    ...
    note.pitch.should eq(note2.pitch)
