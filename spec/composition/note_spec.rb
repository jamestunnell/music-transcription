...
describe Musicality::Note do
  ...
  it "should be constructible (no raised exceptions) without the optional parameters" do
    lambda { Musicality::Note.new :pitches => [@pitch], :duration => 1.to_r }.should_not raise_error ArgumentError
    lambda { Musicality::Note.new :pitches => [@pitch], :duration => 1.to_r }.should_not raise_error RangeError
  end
  
  it "should assign pitch and duration parameters when given during construction" do
    note = Musicality::Note.new :pitches => [@pitch], :duration => 2.to_r
  ...
  it "should assign :loudness, :intensity, and :seperation parameters if given during construction" do
    note = Musicality::Note.new :pitches => [@pitch], :duration => 2.to_r, :loudness => 0.1, :intensity => 0.2, :seperation => 0.3
  ...
  it "should assign :tie and parameter if given during construction" do
    note = Musicality::Note.new :pitches => [@pitch], :duration => 2.to_r, :tie => false
    note.tie.should be_false

    note = Musicality::Note.new :pitches => [@pitch], :duration => 2.to_r, :tie => true
